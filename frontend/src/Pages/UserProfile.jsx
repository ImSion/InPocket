...
export default function UserProfile() {
  ...
  useEffect(() => {
    if (userData) {
      setFormData({
        ...
        data_di_nascita: userData.data_di_nascita ? new Date(userData.data_di_nascita).toISOString().split('T')[0] : '',
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const updatedUser = await updateUser(userData.email, formData);
